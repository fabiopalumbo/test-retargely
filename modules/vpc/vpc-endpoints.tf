...
resource "aws_vpc_endpoint_route_table_association" "custom_s3" {
  count = var.create_vpc && var.enable_s3_endpoint && length(var.s3_endping_route_tables) > 0 ? length(var.s3_endping_route_tables) : 0

  vpc_endpoint_id = aws_vpc_endpoint.s3[0].id
  route_table_id  = element(var.s3_endping_route_tables, count.index)
}
