...
resource "aws_cloudwatch_log_stream" "dynamo_logs_stream" {
  count = var.create_logstream ? 1 :0 
  name           = "${var.function_name}-dynamo-controller-${random_string.this.id}"
  log_group_name = aws_cloudwatch_log_group.lambda[0].name
}

resource "aws_lambda_permission" "aws_lambda_permission" {
  count = var.create_logstream ? 1 :0 
  statement_id   = "AllowExecutionFromCloudWatchLog"
  action         = "lambda:InvokeFunction"
  function_name  = var.logstreamer_lambda_arn
  principal      = "logs.${data.aws_region.current.name}.amazonaws.com"
  source_account = data.aws_caller_identity.current.account_id
  source_arn     = local.log_group_arn
}

resource "aws_cloudwatch_log_subscription_filter" "logstreamer_es_subscription" {
  count = var.create_logstream ? 1 :0 
  name            = "${var.function_name}-logstreamer-${random_string.this.id}"
  log_group_name  = aws_cloudwatch_log_group.lambda[0].name
  filter_pattern  = ""
  destination_arn = var.logstreamer_lambda_arn
}
